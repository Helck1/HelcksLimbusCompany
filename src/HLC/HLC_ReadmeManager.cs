...
namespace LimbusLocalize
{
    public static class HLC_ReadmeManager
    {
            ...
            {
                {
                    "Action_Issues",()=>
                    {
                        ...
                        Application.OpenURL(LCB_HLCMod.HLCLink + "/issues?q=is:issue");
                    }
                }
            };
        }
        ...
        public static void InitReadmeSprites()
        {
            ...
            foreach (FileInfo fileInfo in new DirectoryInfo(LCB_HLCMod.ModPath + "/Localize/Readme").GetFiles().Where(f => f.Extension == ".jpg" || f.Extension == ".png"))
            {
        ...
        {
            ReadmeList.Clear();
            foreach (var notices in JSONNode.Parse(File.ReadAllText(LCB_HLCMod.ModPath + "/Localize/Readme/Readme.json"))[0].AsArray.m_List)
            {
        ...
        {
            HLC_UpdateChecker.CheckReadmeUpdate();
